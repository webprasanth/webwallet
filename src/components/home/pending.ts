...
export default class HomePending extends BaseElement {
  ...
  mounted() {
    ...
    HomePending.unsubscribe = store.subscribe(
  ...
  updateAll() {
    ...
    this.currentActiveTabId =
      currentTab == TAB.OUTGOING ? TAB.INCOMING : TAB.OUTGOING;
    this.loadData();
  ...
  initDatePickers(showAll: boolean) {
    ...
    if (!this.fromDateObject) {
      this.fromDateObject = $('#pendingFromDate')
        .parent()
        .datepicker({
    ...
    if (!this.toDateObject) {
      this.toDateObject = $('#pendingToDate')
        .parent()
        .datepicker({
          format: this.DATE_PICKER_FORMAT,
          autoclose: true,
    ...
    if (showAll) {
      this.fromDateObject.datepicker(
        'setDate',
        new Date(new Date(state.userData.user.created_ts).setHours(0, 0, 0, 0))
      );
    } else {
      let s = this.getDisplayDate(
        new Date(state.userData.user.created_ts),
        state.userData.user.timezone
      );
      let timeSignup = new Date(s);

      let to = new Date(
        this.getDisplayDate(new Date(), state.userData.user.timezone)
      ).getTime();
    ...
    this.fromDateObject.datepicker(
      'setEndDate',
      this.toDateObject.datepicker('getDate')
    );
  ...
  getPendingRequestStatus(status, type): string {
    switch (status) {
      case 0:
        return type == 1
          ? this.getText('pending_request_status_waiting')
          : this.getText('pending_request_status_pending');
      case 1:
        return this.getText('pending_request_status_paid');
      case 2:
        return this.getText('pending_request_status_denied');
      case 3:
        return this.getText('pending_request_status_cancelled');
      default:
        return '';
    }
  }
  ...
  showDetail(event: FCEvent) {
    ...
    let opts = {
      detail: event.item,
      pendingRequestStatus: this.getPendingRequestStatus(
        event.item.status,
        event.item.type
      ),
      cancelCb,
