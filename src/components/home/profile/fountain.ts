...
export default class FountainSetting extends BaseElement {
  ...
  mounted() {
    ...
    FountainSetting.unsubscribe = store.subscribe(
  ...
  hasFountainSetting() {
    return !!(
      ...
      this.settings.domainStr.length > 0
    );
  }
    ...
    if (this.userProfile.auth_version == 3) {
      ...
      if (!this.userProfile.fountainSecret) {
        riot.mount('#confirm-send', 'request-password', {
          cb: this.enableFountain.bind(this),
        });
        return;
      }
      ...
      if (userKey) {
        let privKeyHex = Premium.xaesDecrypt(
          password,
          userKey.encryptedPrivKey
        );
    ...
    if (domains.length > 0) {
      ...
      for (let i = 0; i < domains.length; i++) {
        ...
        if (!domains[i].match(domainReg)) {
          super.showError(
            '',
            domains[i] + this.getText('fountain_invalid_domain_msg')
          );
          return null;
        }
      }
