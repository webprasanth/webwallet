...
export default class MobileLogin extends BaseElement {
  ...
  mounted() {
    ...
    MobileLogin.unsubscribe = store.subscribe(
  ...
  onApplicationStateChanged() {
    ...
    switch (type) {
      ...
      case USERS.LOGIN_FAILED:
        super.showError(
          this.getText('login_cas_login_fail_err'),
          this.getText('login_incorrect_usernamepassword_msg')
        );
        break;
      case USERS.NEED_VERIFY_GOOGLE_2FA:
        riot.mount(
          '#confirm-send',
          'twofa-verification-dialog',
          data.loginData
        );
        break;
      default:
  ...
  onSignupButtonClick(event: Event) {
    if (
      this.userLocation.info.country_code == 'US' &&
      this.userLocation.info.region_code == 'NY'
    ) {
      let message = this.getText('login_usa_limitted_msg');
      riot.mount('#error-dialog', 'location-error', {
        title: 'Error',
        message: message,
      });
      return;
  ...
  onSignupFail(resp) {
    ...
    if (resp.status == 'EMAIL_IN_USED') {
      super.showError(
        this.getText('common_label_error_title'),
        this.getText('signup_email_already_exist')
      );
