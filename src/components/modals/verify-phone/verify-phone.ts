...
export default class VerifyPhone extends BaseElement {
  ...
  constructor() {
    ...
    VerifyPhone.unsubscribe = store.subscribe(
  ...
  mounted() {
    tag = this;
    $('#verify-phone-body #not_get_code_text').html(
      this.getText('phone_verify_not_get_code_title')
    );
  ...
  onApplicationStateChanged() {
    ...
    switch (actionType) {
      case PROFILE.VERIFY_PHONE_SUCCESS:
        ...
        store.dispatch(
          profileActions.updateProfileSuccess({ profile: profile })
        );
        break;
